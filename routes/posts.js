...
router.get('/post/:post_id', async (req, res) => {
    try {
        const post_id = req.params.post_id; // Get the post ID from the request parameters
      console.log('post_id ',post_id)
        const posts = await postsCollection.aggregate([
            {
                // Match the specific post ID
                $match: {
                    _id: { $eq: new ObjectId(post_id) } // Filter for the specific post ID
                }
            },
            {
                $addFields: {
